		...
		if(this.vpb_files.length > 0)
		{
 			...
 			for(var i = 0; i<this.vpb_files.length; i++)
			{
				...
				if(typeof this.vpb_files[i] != undefined && this.vpb_files[i].name != "")
				{
					...
					vpb_added_files_displayer += '<tr id="add_fileID'+vpb_file_id+'" class="'+new_classc+'">'+
													'<td>'+vpb_file_icon+' '+this.vpb_files[i].name.substring(0, 40)+'</td>'+
													'<td><span id="uploading_'+vpb_file_id+'"><span style=color:blue;>Ready</span></span></td>'+
													'<td>'+vpb_actual_fileSize+'</td>'+
													'<td><span id="remove'+vpb_file_id+'"><span class="vpb_files_remove_left_inner" onclick="vpb_remove_this_file(\''+vpb_file_id+'\',\''+this.vpb_files[i].name+'\');">Remove</span></span></td></tr></div>';
								...
								if ( fileType_response_brought != -1) {
									...
									if ( filesize_response_brought != -1) {
										...
										if ( general_response_brought != -1) {
											alert('Sorry, the file size is bigger than request...');
										}
										else { /* Do nothing */}
									}
								}
